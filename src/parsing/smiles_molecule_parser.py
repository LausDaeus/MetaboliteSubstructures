...
from src.molecule_graphs.molecule import BondType
from src.molecule_graphs.molecule import Molecule
...
class Parser(object):
    ...
    def parse_smiles(self, smiles):
        ...
        for a in tokens:
            d = a.groupdict()
            #print d
            #magic = [(x,d[x]) for x in d if d[x]]
            #print magic
            #print molecule.graph_info()
    ...
    def _square_atom(self, token, molecule):
        ...
        if d['aromatic'] is not None:
            atom = molecule.add_atom(d['element'], d['isotope'], hcount, charge, aromatic=True)
    ...
    def _organic(self, token, molecule):
        ...
        if d['oaromatic'] is not None:
            ...
            atom = molecule.add_atom(d['organic'], aromatic=True)
            self._add_bond_to_aromatic_atom(atom, molecule)
        else:
    ...
    def _add_bond(self, atom, molecule):
        ...
        if self._previous_atom is not None:
            if self._previous_bond is None:
                new_bond = molecule.add_bond(self._previous_atom, atom, BondType.single,'-')
            if self._previous_bond is not None:
                d = self._previous_bond.groupdict()
                if d['single'] is not None:
                    new_bond = molecule.add_bond(self._previous_atom, atom, BondType.single, d['single'])
                elif d['double'] is not None:
                    new_bond = molecule.add_bond(self._previous_atom, atom, BondType.double, d['double'])
                elif d['triple'] is not None:
                    new_bond = molecule.add_bond(self._previous_atom, atom, BondType.triple, d['triple'])
                elif d['quadruple'] is not None:
                    new_bond = molecule.add_bond(self._previous_atom, atom, BondType.quadruple, d['quadruple'])
                elif d['arom_bond'] is not None:
                    new_bond = molecule.add_bond(self._previous_atom, atom, BondType.aromatic, d['arom_bond'])
    ...
    def _add_bond_to_aromatic_atom(self, atom, molecule):
        ...
        if self._previous_atom is not None:
            if self._previous_atom.aromatic:
                molecule.add_bond(self._previous_atom, atom, BondType.aromatic, '-')
            elif self._previous_atom is not None and not self._previous_atom.aromatic:
                molecule.add_bond(self._previous_atom, atom, BondType.single, '-')
    ...
    def _ring(self, token, molecule):
        ...
        if number not in self._break_points:                  # The number has not been encountered yet (open ring)
            ...
            del self._break_points[number] #Remove number as it may be re-used
            if ring_bond is None and self._previous_bond is None:    # No bond symbol has been specified
                if ring_atom.aromatic:
                    ...
                    molecule.add_bond(self._previous_atom, ring_atom, BondType.single, '-')
